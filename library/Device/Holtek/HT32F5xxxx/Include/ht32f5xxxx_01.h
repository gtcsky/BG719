...
/**
 * @brief Sigma Analog to Digital Converter
 */
typedef struct
{
	  __IO uint8_t PWRC;              		/*!< 0x000          	Sigma ADC Power Control Register                     */
	  __IO uint8_t PGAC0;              	/*!< 0x001         	PGA Control Register 0                         			*/
	  __IO uint8_t PGAC1;              	/*!< 0x002         	PGA Control Register 0                         			*/
	  __IO uint8_t PGACS;              	/*!< 0x003         	PGA Channel Select Register                       		*/
	  __IO uint8_t ADRL;              		/*!< 0x004         	Sigma AD Result Low Register                         	*/
	  __IO uint8_t ADRM;              	/*!< 0x005         	Sigma AD Result Middle Register                        */
	  __IO uint8_t ADRH;              		/*!< 0x006         	Sigma AD Result High Register                         	*/
	  __IO uint8_t ADCR0;              	/*!< 0x007         	Sigma ADC Conversion Control Register0           */
	  __IO uint8_t ADCR1;             		/*!< 0x008         	Sigma ADC Conversion Control Register1           	*/
	  __IO uint8_t ADCS;              		/*!< 0x009         	Sigma ADC Clock Select  Register                        */
	  __IO uint8_t ADCTE;            		/*!< 0x00A         	reserve                       						*/
	  __IO uint8_t DAH;              		/*!< 0x00B         	DA output code High	Register                         	*/
	  __IO uint8_t DAL;              		/*!< 0x00C         	DA output code Low  Resister	                       	*/
	  __IO uint8_t DACC;              		/*!< 0x00D         	DA control Resister                         			*/
	  __IO uint8_t SIMC0;              		/*!< 0x00E         	Debounce Time   Control Register                       */
	       uint8_t RESERVE0[1];     		/*!< 0x00F         Reserved                                                			*/
	  __IO uint8_t SIMTOC;           		/*!< 0x010         	I2c Time Out control Resister                      		*/
	  __IO uint8_t HIRCC;              		/*!< 0x011         	HIRC Control Resister                         			*/
	
}HT_Sigma_Adc_TypeDef;
